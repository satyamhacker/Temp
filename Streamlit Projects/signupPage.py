...
if submit_button:
    if not email or not password:
        ...
        if not is_valid_password:
            ...
            try:
                response = requests.post("http://localhost:3000/signup", json=payload)
                if response.status_code == 201:
                    st.success(f"You are Singup Successfully");
                    # st.markdown("[Go to Login page](./pages/loginPage.py)")
                    st.markdown("[Go to Login page](http://localhost:8501/loginPage)")
